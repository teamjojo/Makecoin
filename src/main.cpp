...
#if defined(NDEBUG)
# error "Makecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0b37be6468d486ef2b308848feeb0a15a781c62b04318127d1714bfdb1b40456");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Makecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Makecoin Signed Message:\n";
...
{
    // Makecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Makecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Makecoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Makecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Makecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Makecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x535db6ee8e8e8ad6d381fae3f35327aee5bfcf30e06aab988fe10dc94c1fa6f8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048a2a0f9d55d2b4010f4554829543e1ca98768ea48bd2048544b9f6d771bffc9e51f284227ac5ab372b60cfb6b7bc3b54d7ee598f50ca594f11b47dfd7469fdeb") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1536502172;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086055658;

        if (fTestNet)
        {
            block.nTime    = 1536502085;
            block.nNonce   = 387641792;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("e84d1242e0d4ce366a77bb0533c840b3dfabd1dac37a39a4501943151b0db9c1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc5, 0xba, 0xd3 }; // Makecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MakecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MakecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MakecoinMiner(CWallet *pwallet)
{
    printf("MakecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("makecoin-miner");
    ...
    try { loop {
        ...
        printf("Running MakecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MakecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MakecoinMiner, pwallet));
}
